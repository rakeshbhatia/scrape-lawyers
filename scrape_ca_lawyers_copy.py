...
class AttorneyScraper:
	...
	def scrape_da_score1(self, record):
		#print("Scraping DA score for url: ", record[3])
		print("Scraping entry: ", record[0])
		...
		search.send_keys("http://" + record[3])
		...
		if da_score.strip() == "":
			print("DA score empty")
			search = driver.find_element(By.ID, "url")
			search.send_keys("https://" + record[3])
		...
		# Append data to CSV file
		df.to_csv("ca-law-firms-with-website-3.csv", mode="a", index=False, header=False)

	def scrape_all_da_scores1(self):
		print("Scraping all DA scores")

		# Load law firm links
		df = pd.read_csv("ca-law-firms-with-website-2.csv")
		df = df.astype(str)

		# Convert df to list of tuples
		records = list(df.to_records(index=True))[1684:]

		for record in records:
			self.scrape_da_score1(record)	

	def scrape_all_da_scores1_alt(self):
		print("Scraping all DA scores")
		...
		with Pool(processes=8) as pool:
			# Issue tasks to process pool
			results = pool.imap(self.scrape_da_score1, records)
			# Shutdown process pool
			pool.close()
			# Wait for all issued tasks to complete
			pool.join()
	...
	def scrape_da_score2(self, url_string):
		...
		# Append data to CSV file
		df.to_csv("da-scores-1.csv", mode="a", index=False, header=False)

	def scrape_all_da_scores2(self):
		print("Scraping all DA scores")

		# Load law firm links
		df = pd.read_csv("ca-law-firms-with-website-1.csv")
		df = df.astype(str)

		urls = df["Website"].tolist()

		for i in range(1760, len(urls), 10):
			url_string = ",".join(urls[i:i+10])
			self.scrape_da_score2(url_string)

	def scrape_da_score3(self, url_string):
		#print("Scraping entry: ", record[0])
		#print("Scraping url: ", record[3])
		...
		element = WebDriverWait(driver, 30).until(EC.presence_of_element_located((By.CLASS_NAME, "display table")))
	...
	def scrape_all_da_scores3(self):
		print("Scraping all DA scores")
		...
		with Pool(processes=8) as pool:
			# Issue tasks to process pool
			results = pool.imap(self.scrape_da_score3, records)
			# Shutdown process pool
			pool.close()
			# Wait for all issued tasks to complete
			pool.join()

	def scrape_all_da_scores3_old(self):
		print("Scraping all DA scores")
		...
		for i in range(0, len(urls), 20):
			time.sleep(random.randint(0, 3))
			url_string = "\n".join(urls[i:i+20])
			self.scrape_da_score3(url_string)

	def scrape_da_score_alt(self, record):
		print("Scraping entry: ", record[0])
		print("Scraping url: ", record[3])
		...
		#search = driver.find_element(By.NAME, "urla")
		search = driver.find_element(By.ID, "fdgdf")
		search.send_keys("http://" + record[3])
		...
		button = driver.find_element(By.CLASS_NAME, "btn btn-info marg")
		button.click()
		...
		xpath = "/html/body/div/div[2]/div/article/div/div/div[1]/table/tbody/tr[2]/td[3]"
		da_score = driver.find_element(By.XPATH, xpath).text
		print("Domain Authority: ", da_score)
	...
	def scrape_all_da_scores_alt(self):
		print("Scraping all DA scores")
		...
		# Convert df to list of tuples
		records = list(df.to_records(index=True))[:10]

		for record in records:
			self.scrape_da_score(record)
	...
	def final_processing(self):
		df = pd.read_csv("ca-law-firms-with-website-6.csv")
		#df = df.astype(str)

		df = df.sort_values(by=["City"], ignore_index=True)

		df.to_csv("ca-law-firms-with-website-7.csv", index=False)
